...
implemented. These subsystems must meet the functional requirements described in
...
\ref{sub:Agda Tree CLI} to make sure the usability non-functional requirement is
...
for outward edges, check for inward edges with the command:
...
subsection \ref{sub: Building Module Tree}, using Agda's built-in dependency
...
\ref{sub:Agda Tree CLI}, argparse is used to cream the CLI. argparse will parse
...
\marginpar{Write about switching from individual modules to index files for performance}
